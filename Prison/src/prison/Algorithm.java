...
public class Algorithm {
    ...
    int cellWidth; // 8 bit? zakres 0-127
    int cellHeight; // 8 bit? zakres 0-127
    int cellQuantity; // 4 bit? zakres 0-15
    int bedQuantity; // 6 bit? zakres 0-63
    int cameraQuantity; // 7 bit? zakres 0-63
    //
    // Wygląd klucza (indeksy) : 
    // keySize = 72
    // 0-8 | 9-17 | 18-26 | 27-35 | 36-43 | 44-51 | 53-55 | 56-61 | 63-67 |
    ...
    double cellQuantityParameter(int maxCellQuantity, int cellQuantity) {
        ...
        if (cellQuantity > maxCellQuantity) {
            return -50;//-1*((double)cellQuantity/(double)maxCellQuantity-1); // np: 12/4 zwroci jako -2, 5/4 zwroci jako -0.2 itd...
        }
    ...
    double cameraQuantityParameter(int optimalCameraQuantity, int cameraQuantity) {
        ...
        if (cameraQuantity > optimalCameraQuantity) {
            return -1 * ((double) cameraQuantity / (double) optimalCameraQuantity - 1);
        } 
        return (double) cameraQuantity / (double) optimalCameraQuantity;

    }

    double budgetParameter(int budget, int cellQuantity, int bedQuantity, int cameraQuantity) {

        if (budget == 0) {
            return 0;
        }

        int money = cameraQuantity * 10 + bedQuantity * 5 + cellQuantity * 10;
 /*       if (money > budget) {
            return -1 * ((double) money / (double) budget - 1);
        } */
    ...
    double densityParameter(int X1, int H1, int X2, int H2, int cellWidth, int cellHeight, int cellQuantity, int maxCellQuantity) {
        ...
        if(cellQuantity>maxCellQuantity || prizonArea<cellArea) return 0;
    ...
    int maxBedQuantity(int cellQuantity, int cellWidth, int CellHeight) {
        int max = cellQuantity * (2 * (cellHeight / (bedHeight + evacuationRouteWidth))
                * (((cellWidth - evacuationRouteWidth) / 2 - bedWidth) / (bedWidth + evacuationRouteWidth)) - 1);
        if (max < 0) {
            max = 0; // chodzi o to żeby nie było ujemnych wartości
    ...
    int optimalCameraQuantity(int X1, int H1, int X2, int H2,
            int cellQuantity, int cellWidth, int cellHeight, int camerasPerCell) {
        int optimal;
        int cellsCameraQuantity = cellQuantity * camerasPerCell;
    ...
    public double getFitness(String wynik) {
        ...
        double fitness = 0;
        interpreteKey(wynik);
        maxBedQuantity = maxBedQuantity(cellQuantity, cellWidth, cellHeight);
        maxCellQuantity = maxCellQuantity(X1, H1, X2, H2, cellWidth, cellHeight);
        optimalCameraQuantity = optimalCameraQuantity(X1, H1, X2, H2, cellQuantity, cellWidth, cellHeight, camerasPerCell);
        
        
        if (prizonShapeParameter(X1, H1, X2, H2, cellWidth, cellHeight) == 0) {

            fitness -= 100; // do ustalenia
        ...
        fitness += cellQuantityParameter(maxCellQuantity, cellQuantity)
                ...
                + budgetParameter(budget, cellQuantity, bedQuantity, cameraQuantity)
                + densityParameter(X1, H1, X2, H2, cellWidth, cellHeight, cellQuantity, maxCellQuantity);
    ...
    public void print(String key) { // tylko do testów
        ...
        System.out.println(popNumber + ". X1: " + X1 + " H1: " + H1 + " X2: " + X2 + " H2: " + H2
                + " cW: " + cellWidth + " cH: " + cellHeight + " cellQ " + cellQuantity
                + " bedQ: " + bedQuantity + " camQ: " + cameraQuantity + " fit: " + getFitness(key));
